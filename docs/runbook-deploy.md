...
> Узел `fels` развёрнут иначе: там есть графика, нет LUKS, автовход и `sudo NOPASSWD`
> ([ADR 0008](adr/0008-multi-purpose-host.md), [ADR 0009](adr/0009-no-full-disk-encryption.md)).
> Что из этого закрывается без переустановки, что требует окна обслуживания, а что
> принято как риск — [runbook-host-hardening.md](runbook-host-hardening.md). Состояние
> среды проверяется читающим тестом `sudo tests/acceptance/a18-host-env.sh`.
...
# Вердикт по бэкапу входит в общий state, но не выше Degraded: на новом узле, где
# ночная задача не отработала ни разу, общий state — degraded, а поле backup — down.
# Первый прогон — hearthctl backup now.
hearthctl health
```

## 5а. Конфигурация: что останавливает старт, а что только предупреждает

`hearthd check` читает `/etc/hearth/hearthd.toml` и отвечает одним из трёх способов.

**Отказ.** Демон не поднимется, пока не починят. Так закрыты только те случаи, где
иначе теряется защита или узел заведомо неработоспособен: чужой (неразборчивый)
отпечаток в `api.allowed_admin_fingerprints`, `api.listen` или адрес алертов вне
`node.lan_networks`, два сервиса на одном порту, пустые `backup.recipients`.

**Предупреждение.** Узел работает, а строка уходит в журнал и в алерты (`hearthctl
alerts`) при каждом старте. Сейчас так обрабатывается ровно один случай:

```
ВНИМАНИЕ: backup.required_paths содержит /var/opt/nowhere, которого нет в
backup.paths: этот путь не архивируется, и требование невыполнимо...
```

Это опечатка в одном из двух списков секции `[backup]`. Отказом старта она была бы
несоразмерна себе: релеи продолжают работать и в этом случае, а вот надзор, проверка
целостности, бэкап, admin API и алерты — нет, и снаружи это ничем не видно. Хуже того,
счёт пришёл бы не в момент правки, а при следующей перезагрузке — конфигурация
читается только на старте. Поэтому узел живёт, но молчать об этом не может: вдобавок к
алерту ночной прогон бэкапа будет проваливаться каждую ночь («не вошло в архив») и
держать `backup` в `Down`.

Чинится правкой одного из списков и `systemctl restart hearthd`. Проверить до
перезапуска: `hearthd check` — он печатает те же предупреждения.

**Тишина.** Замечаний нет.
