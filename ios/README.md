...
| Приложение запускается и работает на устройстве | проверено 2026-09-23: iPhone 13 Pro, iOS 26.6 — запускается, ядро живо, подключается к узлу |
| `build-core.sh` | проверено 2026-09-21: GitHub Actions, `macos-15`, 3 ч 7 мин, ядро arm64 с патчем 0101 внутри |
| Подпись, архив, загрузка | проверено: сборки 1–4 приняты App Store Connect (VALID); 4 — первая с подключением к узлу, оранжевой темой и без SimpleX в текстах |
...
## «Ошибка двоичного файла» без объяснений: номер версии

Сборки 2, 3 и 4 отклонялись одинаково — статус `INVALID_BINARY` через минуты после
отправки, без текста в сообщениях проверки и без письма. Сами файлы были в порядке:
`altool --validate-app` проходил, подпись, права, архитектуры, манифесты — всё чисто,
сборки получали `VALID`.

Причина была не в файле, а в расхождении с карточкой: `MARKETING_VERSION` в проекте —
`7.0.1` (от upstream), а версия в App Store Connect была заведена как `1.0`. Apple требует,
чтобы `CFBundleShortVersionString` сборки совпадал с номером отправляемой версии;
загрузка это не проверяет, проверяет отправка — и молча ставит «Ошибка двоичного файла».

Версия в App Store Connect переименована в `7.0.1` — совпадает с Android (`7.0.1-h24`) и
с тегом upstream. **При каждой новой версии номер в карточке и `MARKETING_VERSION` в
`project.pbxproj` должны совпадать**; `build-release.sh` берёт его из проекта как есть.

## ATS: почему узел был недоступен с телефона

Приложение на устройстве не могло подключиться к узлу — `NSURLErrorSecureConnectionFailed`
(`-1200`) на любом интернете, при том что Android работает, а `swift`-утилита с того же
мака подключается.

Причина — **App Transport Security**. Раздела `NSAppTransportSecurity` в `Info.plist`
нет ни у upstream, ни, до этого, у нас. При умолчаниях iOS проверяет сертификат сервера
**сама и раньше**, чем спросит делегат. Сертификат узла подписан частным
`hearth admin CA`, системе неизвестным, — соединение обрывается, а
`HearthPinningDelegate` не вызывается вовсе. Отсюда и обманчивая картина: на Android ATS
не существует, на консольную утилиту macOS не распространяется, от сети не зависит.

Исключение выдаёт `bake-node.sh` — на тот же хост, что вшивает, во все три `Info.plist`
(приложение, NSE, Share Extension). Держать его руками нельзя: разойдётся с узлом.

Что именно отключается: **только системная проверка доверия** для одного хоста. Её
заменяет наш пиннинг, который строже — `SecTrustSetAnchorCertificatesOnly` к вшитому CA
плюс совпадение имени. `NSExceptionMinimumTLSVersion` задан как `TLSv1.2` явно, forward
secrecy остаётся обязательным по умолчанию, HTTP не разрешается: схема `https`.

Проверено на устройстве 2026-09-23: iPhone 13 Pro, iOS 26.6 — приложение подключается к
узлу и проходит заведение по коду доступа.
