...
## «Ошибка двоичного файла» без объяснений — причина не найдена

Отправки сборок 2, 3 и 4 отклоняются одинаково: `INVALID_BINARY` через минуты после
отправки, без текста в сообщениях проверки. Сами сборки при этом `VALID` и
`APP_STORE_ELIGIBLE`, `altool --validate-app` проходит, подпись, права, архитектуры,
манифесты конфиденциальности, `CFBundleIconName` — всё проверено и чисто. На устройстве
приложение работает.

Проверенные и **отвергнутые** версии:

- манифест конфиденциальности и `CFBundleIconName` — добавлены в сборке 3, отказ остался;
- номер версии: в проекте `7.0.1`, карточка была `1.0`. Карточка переименована в `7.0.1`
  (так и держать — совпадает с Android и тегом upstream), отправка 2026-09-23 с совпадающими
  номерами отклонена так же. Совпадение номеров обязательно, но причиной отказа не было.

Причину знает только Apple: объяснение `INVALID_BINARY` приходит письмом держателю
учётной записи, а если его нет — через «Связаться с нами» в App Store Connect.
